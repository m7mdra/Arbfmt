package com.m7mdra.arbfmt.panel;
...
public class EditorTextPanel extends NonOpaquePanel implements EditorTextComponent, DocumentListener, DataProvider, TextAccessor,
    ...
    public EditorTextPanel(@NotNull String text) {
        this(EditorFactory.getInstance().createDocument(text), null, FileTypes.PLAIN_TEXT);
    }

    public EditorTextPanel(@NotNull String text, Project project, FileType fileType) {
        this(EditorFactory.getInstance().createDocument(text), project, fileType, false, true);
    }

    public EditorTextPanel(Document document, Project project, FileType fileType) {
        this(document, project, fileType, false, true);
    }

    public EditorTextPanel(Project project, FileType fileType) {
        this(null, project, fileType, false, false);
    }

    public EditorTextPanel(Document document, Project project, FileType fileType, boolean isViewer) {
    ...
    private void initOneLineMode(final EditorEx editor) {
        ...
        EditorColorsScheme customGlobalScheme = editor.getColorsScheme();
    ...
    private static class Jdk7DelegatingToRootTraversalPolicy extends AbstractDelegatingToRootTraversalPolicy {
        ...
        public Component getDefaultComponent(Container aContainer) {
            ...
            Editor editor = aContainer instanceof EditorTextPanel ? ((EditorTextPanel) aContainer).getEditor() : null;
